...
/**
 * Draws an edge and a Nash flow. The Nash flow is stored in values for the different iterations of the algorithm.
 *
...
public class GLNashFlowEdge extends GLEdge {
    ...
    /**
     * Draws the edge. The edge is a solid, semi transparent cylinder. At a first step the coordinate system is rotated
     * such that the edge goes in {@code z} direction. Then the edge is drawn.
     *
     * @param gl the graphics object
     */
    private void displayEdge(GL2 gl) {
        gl.glPushMatrix();
        edgeColor.draw(gl);
        wallColor.draw(gl);
    ...
    /**
     * Displays the corridor. The corridor is a transparent cylinder. The back of the cylinder is open, the front of the
     * cylinder is closed but contains a hole of the size of the edge as an exit.
     *
     * @param gl the graphics object
     */
    private void displayCorridor(GL2 gl) {
        ...
        GLU_INSTANCE.gluCylinder(GLU_QUADRIC, width, width, drawnLength * eps, 32, 1);
        gl.glTranslated(0, 0, drawnLength * eps);
        GLU_INSTANCE.gluDisk(GLU_QUADRIC, scale * capacity * 0.5 * eps, width, 32, 1);
        gl.glDisable(GL2.GL_BLEND);
        gl.glPopMatrix();
        gl.glLineWidth(3);
    }
    ...
    /**
     * An auxiliary method that gives the length of the queue depending of the flow data and the current time of
     * visualization.
     *
    ...
    /**
     * <p>
     * Draws the flow (based on the flow data). That means, the flow on one specific edge computed in one iteration of
     * the Nash flow algorithm.</p>
     * <p>
     * The flow consists of three phases.
     * <ul>
     * <li>The first phase consists of the flow entering the edge up to the point where it queues up or leaves the
     * exit.</li>
     ...
     * </ul>
     * There is one special case: when the inflow rate is smaller than the capacity and the flow never queues up and has
     * not to wait, than it is simply drawn as one single piece of flow. </p>
     *
     * @param gl the graphics object
     * @param flowData the flow data displayed
     */
    private void displayFlow(GL2 gl, FlowData flowData) {
    ...
    /**
     * Displays the first phase. The colors used at the start and end point of the phase have to be computed first as
     * they may need information about the other phases colors. The positions of the flow are computed based on the
     ...
     * @param startColorValue the color at the start of the flow (this is where it comes out of the node, which in
     * meanings of time is the end of the flow!)
     * @param endColorValue the color at the end of the flow (this is the piece of flow furthest away!)
     */
    private void drawPhase1(GL2 gl, FlowData flowData, double shownQueueLength, double startColorValue, double endColorValue) {
    ...
    /**
     * Draws the second phase of the flow, that is the queue (if it exists). The colors used at the start and end point
     * of the phase have to be computed first as they may need information about the other phases colors. The positions
     * of the flow are computed based on the current time and the times where flow queues up from the flow data.
     *
     * @param gl the graphics object
     * @param flowData the flow data for the current iteration
     * @param shownQueueLength the length of the queue at this time for this iteration
     ...
     * @param endColorValue the color at the end of the flow (this is the piece of flow furthest away!)
     */
    private void drawPhase2(GL2 gl, FlowData flowData, double shownQueueLength, double startColorValue, double endColorValue) {
        if (currentTime >= flowData.firstEnterExit && currentTime <= flowData.lastLeaveExit) {
    ...
    /**
     * Draws a "simple" flow. That means, a flow that starts at a point at a specific time and ends at an end point at
     * another time. In between the colors scale from the global start value up to the global end value.
     *
     * @param gl the graphics object
     * @param flowData the flow data for the current iteration
     ...
     */
    private void draw(GL2 gl, FlowData flowData, double current, double firstVisible, double lastVisible, double lastInflow, double firstArrival, double relativeStart) {
    ...
    /**
     * Draws a cylinder representing a flow. The cylinder will be colored with a color gradient, whose start and end
     * values have to be submitted.
     *
     * @param gl the graphics object
     ...
     */
    private void drawFlow(GL2 gl, double baseColorValue, double topColorValue, double length, double offset, double flowValue) {
        if (length <= 0) {
            return;
        }
        gl.glPushMatrix();
        ...
        GLU_INSTANCE.gluDisk(GLU_QUADRIC, 0, width, 32, 1);
    ...
    /**
     * Updates the values used to draw the edge. The values are gathered from the associated control class.
     */
    @Override
    public void update() {
