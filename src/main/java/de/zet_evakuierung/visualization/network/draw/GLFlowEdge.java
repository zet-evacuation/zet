...
public class GLFlowEdge extends GLEdge {
    ...
    static double flowThickness = 7 * 0.1;
    /* * 1.5 */ /* 7 */ //* GLFlowGraphControl.sizeMultiplicator; //// factor of 1.5 used for test evacuation report
    static double minFlowThickness = 10 * 0.1;
    /* 10 */ // original 3
    ...
    /**
     * Draws a piece of flow. The start and end of the piece can be drawn or not. If they are drawn their color is
     * flowUnitColor.
     *
    ...
    private void drawPieceOfFlowIndividual(GL2 gl, double radius) {
        flowColor.draw(gl);
    ...
    /**
     * <p>
     * Draws the flow on the edge. An edge is divided into parts with equal length, the number of parts equals the
     * (integral) length of the edge. The flow is displayed in different widths, if the flow rate is larger. Flow parts
     * move along the edge with equal speed.</p>
     * <p>
     * Note that flow particles only use the same speed, if the length of the edges is correct also in the graphical
     * representation. The speed of flow is computed based on the network flow model length of the edge, so the
     * positions of the start- and end node for the edge should fit to the model to get a smooth and homogeneous
     * flow.</p>
     *
    ...
    private void drawFlow(GL2 gl) {
        ...
        if (pointer < flowOnEdge.size()) {
            ...
            } else // move further on the z-axis
            {
    ...
    /**
     * Draws the flow on the edge. The edge is already drawn using the
     * {@link #performStaticDrawing(javax.media.opengl.GL)} method.
     *
