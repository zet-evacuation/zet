 ...
 *
 */
public class GLWall extends AbstractDrawable<GLWall, Wall> {
     ...
     */
    public GLWall(Wall control, EvacuationVisualizationModel visualizationModel) {
        super(control);
        basePoints = getBasePoints(control, visualizationModel);
    ...
    private List<GLVector> getBasePoints(Wall visualized, EvacuationVisualizationModel visualizationModel) {
    ...
    /**
     * Calculates the angle between two normalized vectors. The calculation includes the orientation, thus the returned
     * angle is within the interval 0 and 2*PI
     *
     * @param v1 one vector
    ...
    private GLVector getEnd(int segment, int segmentCount) {
        return new GLVector(basePoints.get(mod(segment + 1, segmentCount)));
        //segment < segmentCount ? new GLVector( basePoints.get( segment + 1) ) : new GLVector( basePoints.get( 0 ) );
    }
    ...
    private GLVector getDoorFrontCoordinate(GLVector startVec, GLVector endVec, GLVector startVecHigh,
            GLVector endVecHigh, boolean start, int vecNumber) {
        GLVector ret = new GLVector();
        GLVector doorVector = new GLVector(endVec.sub(startVec));
        doorVector.normalize();
        doorVector.scalarMultiplicateTo(0.1); // improvement!
        //doorVector.normal( gl );
        GLVector normal = model.isRoomLeft()
                ? new GLVector(Vector3.normal(endVec, startVec, startVecHigh))
                : new GLVector(Vector3.normal(startVec, endVec, endVecHigh));
        normal.normalize();
        normal.scalarMultiplicateTo(0.1);
