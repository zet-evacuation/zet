...
public class Visualization<U extends DrawableControlable> extends AbstractVisualization {
    ...
    /**
     * Creates a new instance of the {@code Visualization} panel with given properties in an {@code GLCapabilities}.
     *
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        if (recording == RecordingMode.Recording && frameUsed) {
            ...
            EventServer.getInstance().dispatchEvent(new MessageEvent<>(this, MessageType.VideoFrame, "Video frame " + (++screenshotCounter) + " - " + (screenshotCounter * (1.0 / movieFrameRate)) + " sec"));
            movieManager.addImage(newFilename);
            if (!introRunning) {
                movieStep();
            }
        }
        EventServer.getInstance().dispatchEvent(new MessageEvent<>(this, MessageType.Status, "Frame rendered"));
    }
    ...
    /**
     * Sets the correct animation time. Calculated by the difference from the current time and last time. If the control
     * class states, that the simulation is over, animation is stopped or reseted if looping is activated.
     *
     * @see #getDeltaTime()
     */
    @Override
    ...
    /**
     * Prints some getText on the screen. The getText is shown before the visualization starts if a movie is recorded.
     */
    private void drawIntroText(int index) {
        ...
        for (int i = 0; i < tfs.size(); ++i) //			font.print( 100, this.getHeight() - (7+i) * fontSize, tfs.getText( index ) );
        {
            if (tfs.getBold(i)) {
    ...
    /**
     * Prints some getText on the screen. The getText is shown after the visualization has finished if a movie is
     * recorded.
     */
    ...
    /**
     * Draws the main scene, that is the walls, the cellular automaton, the individuals and the graph. Includes getText
     * visible in the scene but not the copyright notice, etc.
     */
    ...
    protected void drawFPS() {
        ...
        // compute time:
        double secs = Conversion.NANO_SECONDS_TO_SEC * getTimeSinceStart();
    ...
    /**
     * Converts rendered frames to a movie or stops creating a movie. It is called if the visualization is finished (CA
     * and graph finished). Call it, if you stop movie recording in between.
     */
    ...
    /**
     * <p>
     * Enables or disables the movie capture mode and sets the movie resolution if recording is enabled and resets back
     * to normal resolution if recording is disabled.</p>
     * <p>
     * If called with {@code recording} set to true the current resolution is stored and rewritten if called with false.
     * Thus automatic resetting of the resolution only works if the method is called with true and false in this
     * order.</p>
     *
    ...
    /**
     * Only use this method to pause recording (set recording to SkipFrame), no intro times are reset and the movie
    ...
    public void initGFX(GLAutoDrawable drawable) {
        ...
        gl.glHint(GL2.GL_PERSPECTIVE_CORRECTION_HINT, GL.GL_NICEST);		// Perspective calculations with high precision
