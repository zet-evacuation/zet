...
public class JGraphStatisticPanel extends JPanel {
  ...
  private void createTestInstance() {
    ...
    for( int index = 0; index < numberOfRuns; index++ ) {
      ...
      int[] sup = new int[3];
      sup[0] = 1 + rng.nextInt( 7*numberOfRuns );
      sup[1] = 1 + rng.nextInt( 7*numberOfRuns );
      sup[2] = 1 + rng.nextInt( 7*numberOfRuns );
      supplies.set( source1, sup[0] );
      supplies.set( source2, sup[1] );
      supplies.set( source3, sup[2] );
      supplies.set( sink3, -sup[0] - sup[1] - sup[2] );

      IdentifiableIntegerMapping<Node> nodeCapacities = new IdentifiableIntegerMapping<>( network.nodeCount() );
      for( Node nd : network.nodes() ) {
        nodeCapacities.set( nd, 0 );
      }

      EarliestArrivalFlowProblem eafp = new EarliestArrivalFlowProblem( capacities, network, nodeCapacities, supersink, sources, 10, transitTimes, supplies );
      SEAAPAlgorithm algo = new SEAAPAlgorithm();
      algo.setProblem( eafp );
      algo.run();

      PathBasedFlowOverTime flow = algo.getSolution().getPathBased();
...
//            nfm.setCurrentAssignment(supplies);
      ...
      selectedEdges.add( n );
      selectedEdges.add( o );
      selectedFlows = new LinkedList<>();
      selectedFlows.add( flow.iterator().next() );
    }
  }

  final void createComponents() {
