 ...
 */
public class GreedyAlgo extends AbstractAlgorithm<NetworkFlowModel, UndirectedForest> {
    ...
    public UndirectedForest runAlgorithm(NetworkFlowModel networkFlowModel) {
        try {
            ...
            supersink = networkFlowModel.getSupersink();
            int numNodes = networkFlowModel.numberOfNodes();
            TransitForEdge = networkFlowModel.transitTimes();
            capForEdge = networkFlowModel.edgeCapacities();
            currentTransitForEdge = new IdentifiableIntegerMapping<>(networkFlowModel.numberOfEdges());
            currentCapForEdge = new IdentifiableIntegerMapping<>(networkFlowModel.numberOfEdges());

            for (Edge edge : networkFlowModel.graph().edges()) {
      ...
      + OriginNetwork.getExactTransitTime(sortededges.get(k)));
            ...
            network = new DefaultDirectedGraph(networkFlowModel.numberOfNodes(), networkFlowModel.numberOfEdges());
            for (Node node : networkFlowModel) {
                network.setNode(node);
            }
            network.setEdges(networkFlowModel.graph().edges());
            ...
            while (sortededges.size() > 0) {
                ...
                if ((dist > t * TransitForEdge.get(currentEdge))
                    ...
                    if ((used[currentEdge.start().id()][currentEdge.end().id()]) == 0) {
                        ...
                        solEdges.add(edge);
                        trans.add(edge, networkFlowModel.getTransitTime(currentEdge));
                        cap.add(edge, networkFlowModel.getEdgeCapacity(currentEdge));
            ...
            IdentifiableCollection<Edge> addEdges = networkFlowModel.graph().incidentEdges(supersink);
