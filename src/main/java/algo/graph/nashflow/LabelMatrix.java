...
public class LabelMatrix {
    ...
    private final double[][] coeffmatrix1;   //for sum_k:k<i  l_k u(E1(Xi,Xk))
    private final double[][] coeffmatrix2;  //for sum_j:j>i l_j u(E(Xi,Xj))
    private final double[][] coeffmatrix3;  //for sum_k:k<i l_i u(E(Xk,Xi))
    private final double[][] coeffmatrix4;  //for sum_j:j>i l_i u(E1(Xj,Xi))
    private final double[][] coefficientMatrix;

    private final double[] rhs;  //rhs for linear equality system

    private final List<Node>[] nodePartitionArray;
    private final HashMap<Node, Integer> nodeToArrayPosition;
    private final ListSequence<Edge> edges;
    private final List<Edge> E1;
    private final IdentifiableDoubleMapping<Edge> edgeCapacities;
    private final IdentifiableDoubleMapping<Node> nodeDemands;

    public LabelMatrix(NodePartition nodePart, ListSequence<Edge> edgeSet, List<Edge> E_1, IdentifiableDoubleMapping<Edge> edgeCap,
