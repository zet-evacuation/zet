 ...
 */
public abstract class CoordinateTools {
	...
	public static Point translateToModel ( Point p ) {
		return new Point ( zoomInverse (p.x) - xOffset, zoomInverse (p.y) - yOffset );
	}
	
	/** Convenience method, calls zoomInverse (int length). 
	...
	/** Returns the onscreen-length of a line of the given length.
	 * @param length A length in the model measured in millimeters.
	 * @return the onscreen-length
	 */
	 ...
	 * rounding faults zoomInverse (zoom (x)) == x should hold true.
	 */
