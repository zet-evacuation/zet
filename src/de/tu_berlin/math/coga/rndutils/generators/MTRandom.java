...
/**
 * @version 1.0
 * @author David Beaumont, Copyright 2005 <p> A Java implementation of the
 * MT19937 (Mersenne Twister) pseudo random number generator algorithm based
 * upon the original C code by Makoto Matsumoto and Takuji Nishimura (see <a
 * href="http://www.math.sci.hiroshima-u.ac.jp/~m-mat/MT/emt.html">
 * http://www.math.sci.hiroshima-u.ac.jp/~m-mat/MT/emt.html</a> for more
 * information. <p> As a subclass of java.util.Random this class provides a
 * single canonical method next() for generating bits in the pseudo random
 * number sequence. Anyone using this class should invoke the public inherited
 * methods (nextInt(), nextFloat etc.) to obtain values as normal. This class
 * should provide a drop-in replacement for the standard implementation of
 * java.util.Random with the additional advantage of having a far longer period
 * and the ability to use a far larger seed value. <p> This is <b>not</b> a
 * cryptographically strong source of randomness and should <b>not</b> be used
 * for cryptographic systems or in any other situation where true random numbers
 * are required. <p> <!-- Creative Commons License --> <a
 * href="http://creativecommons.org/licenses/LGPL/2.1/"><img alt="CC-GNU LGPL"
 * border="0" src="http://creativecommons.org/images/public/cc-LGPL-a.png"
 * /></a><br /> This software is licensed under the <a
 ...
 * rdf:resource="http://creativecommons.org/licenses/LGPL/2.1/" /> <dc:type
 * rdf:resource="http://purl.org/dc/dcmitype/Software" /> </Work>
 *
 * <License rdf:about="http://creativecommons.org/licenses/LGPL/2.1/"> <permits
 ...
 * rdf:resource="http://web.resource.org/cc/SourceCode" /> </License>
 *
 * </rdf:RDF> -->
 *
 */
public class MTRandom extends Random implements GeneralRandom {
	/**
	 ...
	 * exactly replicating the case where the seed value had not been set prior to
	 * calling genrand_int32. <p> If the compatibility flag is set to true, then
	 * the algorithm will be seeded with the same default value as was used in the
	 * original C code. Furthermore the setSeed() method, which must take a 64 bit
	 * long value, will be limited to using only the lower 32 bits of the seed to
	 * facilitate seamless migration of existing C code into Java where identical
	 * behaviour is required. <p> Whilst useful for ensuring backwards
	 * compatibility, it is advised that this feature not be used unless
	 ...
	 * 64 bit seed value. For a better random number sequence this seed value
	 * should contain as much entropy as possible.
	 *
	 ...
	 * then the sequence of numbers generated by both instances will be identical.
	 * <p> If this instance was initialised in 'compatibility' mode then this
	 * method will only use the lower 32 bits of any seed value passed in and will
	 ...
	 * the genrand_int32 function in the original C code and ensures that using
	 * the standard nextInt() function (inherited from Random) we are able to
	 * replicate behaviour exactly. <p> Note that where the number of bits
	 * requested is not equal to 32 then bits will simply be masked out from the
	 * top of the returned integer value. That is to say that:
	 * <pre>
	 * mt.setSeed(12345);
	 * int foo = mt.nextInt(16) + (mt.nextInt(16) << 16);</pre> will not give the
	 * same result as
	 * <pre>
	 * mt.setSeed(12345);
	 ...
	 * array each time setSeed() is called. <p> If the length of the byte array is
	 * not a multiple of 4 then it is implicitly padded with zeros as necessary.
	 * For example:
	 * <pre>    byte[] { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06 }</pre> becomes
	 * <pre>    int[]  { 0x04030201, 0x00000605 }</pre> <p> Note that this method will
	 * not complain if the given byte array is empty and will produce an empty
	 * integer array, but the setSeed() method will throw an exception if the
	 * empty integer array is passed to it.
	 *
