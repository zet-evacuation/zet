    ...
    public MinSpanningTree runAlgorithm(MinSpanningTreeProblem minspan)
    {
        try{
            ...
            while (sortededges.size()> 0)
            {
                ...
                if ( (dist > t* TransitForEdge.get(currentEdge)) || (capacity > t*capForEdge.get(currentEdge)) || (capacity < (1/t)*capForEdge.get(currentEdge)))
                {
                    if ((capacity > t*capForEdge.get(currentEdge)) || (capacity < (1/t)*capForEdge.get(currentEdge)))
                    {
