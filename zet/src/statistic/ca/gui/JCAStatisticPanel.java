...
public class JCAStatisticPanel extends JPanel {	
	...
	private class BatchResultEntryComboBoxModel extends DefaultComboBoxModel {
		...
		public void setSelectedItem (Object object) {
			...
			if (selectedBatchResultEntry.getCa () != null) {
	...
	private class AddToDiagramPerformed implements ActionListener {
		...
		public void actionPerformed (ActionEvent e) {
			...
			for (Integer i : assignmentIndexToShow) {
				diagramCategoryListModel.addElement (selectedBatchResultEntry.getName () + (PropertyContainer.getInstance().getAsBoolean( "statistic.showAssignmentNamesInDiagrams" ) ?
					"" : " - " + assignmentGroups.get (i).toString ()));
			}
	...
	private void calculateStatistic (String diagrammName) {
		...
		if (diagrammName.equals ("maximale Blockadezeit")) {
			...
			for (int i = 0; i < assignmentIndexToShow.size (); i++) {
				...
				categoryDatasetAssignments.add (selectedBatchResultEntry.getName () + (PropertyContainer.getInstance().getAsBoolean( "statistic.showAssignmentNamesInDiagrams" ) ?
					" - " + assignmentGroups.get (assignmentIndexToShow.get (i)).toString () : ""));
			}
		}
		...
		if (diagrammName.equals ("Distanz über Zeit")) {
			...
			for (int i = 0; i < assignmentIndexToShow.size (); i++) {
				dataset = new XYSeries (selectedBatchResultEntry.getName () + (PropertyContainer.getInstance().getAsBoolean( "statistic.showAssignmentNamesInDiagrams" ) ?
					" - " + assignmentGroups.get (assignmentIndexToShow.get (i)).toString () : ""));
