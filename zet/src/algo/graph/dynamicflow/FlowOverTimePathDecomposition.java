...
public class FlowOverTimePathDecomposition extends Algorithm<ImplicitTimeExpandedResidualNetwork, PathBasedFlowOverTime> {
    ...
    public FlowOverTimePath calculateShortestPath() {
        IdentifiableIntegerMapping distances = new IdentifiableIntegerMapping<Node>(network.numberOfNodes());
        IdentifiableObjectMapping<Node, Edge> preceedingEdges = new IdentifiableObjectMapping<Node, Edge>(network.numberOfEdges(), Edge.class);
        MinHeap<Node, Integer> queue = new MinHeap<Node, Integer>(network.numberOfNodes());
