 ...
 *
 */
public class ZToNonGridGraphConverter {
...
//            ZToGraphRasterSquare square = roomRaster.getSquare((int)Math.floor(XPos/400), (int)Math.floor(YPos/400));
	...
	protected static void createNodes(ZToGraphRasterContainer rasterContainer,
		...
		for (ZToGraphRoomRaster room : rasteredRooms) {
			...
			for (int y = 0; y < numOfRows; y++) {
				for (int x = 0; x < numOfColumns; x++) {
					...
					if (square.accessible() && !square.isMarked()) {
						...
						while (extent < numOfColumns && extent < numOfRows
								&& !downblocked && !rightblocked && !blocked) {
							...
							if (!downblocked && !rightblocked && !blocked) {
								for (int offset = 0; offset <= extent + 1; offset++) {
									ZToGraphRasterSquare rsquare = room
											.getSquare(x + extent + 1, y
													+ offset);
								...
								for (int offset = 0; offset <= extent; offset++) {
									ZToGraphRasterSquare dsquare = room
											.getSquare(x + offset, y + extent
													+ 1);
						...
						while (!downblocked && added < imbalance) {
							...
							for (int offset = 0; offset <= extent; offset++) {
								downblocked |= isDownSquareBlocked(room, x + offset, 
										y + extent + added, accurateDelayAreaCreation, accurateAssignmentAreaCration); 
								 // hinten  nicht+1, da nicht der drunter
							}    // sondern der obere �bergeben werden muss
			...
			if (DebugFlags.NODECREATIONSMALL){
                  ...
                  if (edge != null && start != end){
                        ...
                        if (transitTimes.get(edge) <= 0){
                            ...
                            for(ZToGraphRasterSquare square : nodeToSquare.get(start)){                            
                               if (square.getXOffset() <= startUpperLeftX) startUpperLeftX = square.getXOffset();
                               if (square.getYOffset() <= startUpperLeftY) startUpperLeftY = square.getYOffset();
                               if (square.getXOffset() >= startLowerRightX) startLowerRightX = square.getXOffset();
                               if (square.getYOffset() >= startLowerRightY) startLowerRightY = square.getYOffset();                            
                            }
                            // adapt to rectangle corner coordinates
                           ...
                           for(ZToGraphRasterSquare square : nodeToSquare.get(end)){
                                if (square.getXOffset() <= endUpperLeftX) endUpperLeftX = square.getXOffset();
                                if (square.getYOffset() <= endUpperLeftY) endUpperLeftY = square.getYOffset();
                                if (square.getXOffset() >= endLowerRightX) endLowerRightX = square.getXOffset();
                                if (square.getYOffset() >= endLowerRightY) endLowerRightY = square.getYOffset();                            
                            }
                           // adapt to rectangle corner coordinates
                           endLowerRightX += 400;
                           endLowerRightY += 400;
                           ...
                           if (startLowerRightY == endUpperLeftY) startAboveEnd = true;
                           if (startUpperLeftY == endLowerRightY) startBeneathEnd = true;
                           if (startLowerRightX == endUpperLeftX) startLeftOfEnd = true;
                           if (startUpperLeftX == endLowerRightX) startRightOfEnd = true;
                    ...
                    for(Node nodeB : nodeListOfEndRoom){
                        ...
                        if (edge != null && graph.contains(edge) && doorEdgeToSquare.get(edge) != null && !doorEdgeToSquare.get(edge).isEmpty()){
                            ...
                            for(ZToGraphRasterSquare square : doorSquareListAB){
                                if(square.getNode() == nodeA) doorSquareListA.add(square);
                                if(square.getNode() == nodeB) doorSquareListB.add(square);
                            }
                            int transitTimeA = 0;
                            int transitTimeB = 0;
                            int transitTimeAB;
