...
public class EdgePopupListener implements ActionListener {
	...
	public void actionPerformed (ActionEvent e) {
		try {
			if (e.getActionCommand ().equals ("makePassable")) {
				...
				if (myEdge instanceof RoomEdge) {
					if (GUIOptionManager.getEditMode () != EditMode.TeleportEdgeCreation) {
						...
						EventServer.getInstance ().dispatchEvent (new MessageEvent (
								this, MessageEvent.MessageType.Status,
					...
					EventServer.getInstance ().dispatchEvent (new MessageEvent (
							this, MessageEvent.MessageType.Error,
							"Nur Raumbegrenzungen können zu Stockwerksdurchgängen gemacht werden!"));
				}
			} else if (e.getActionCommand ().equals ("makeEvacEdge")) {
				if (myEdge instanceof RoomEdge) {
					JEditor.getInstance ().getEditView ().getProject ().getPlan ().getDefaultFloor ().addEvacuationRoom ((RoomEdge) myEdge);
				} else {
					EventServer.getInstance ().dispatchEvent (new MessageEvent (
							this, MessageEvent.MessageType.Error,
							"Nur Raumbegrenzungen können zu Evakuierungsausgängen gemacht werden!"));
				}
			} else if (e.getActionCommand ().equals ("createPassageRoom")) {
				if (GUIOptionManager.getEditMode () == EditMode.TeleportEdgeCreation) {
					EventServer.getInstance ().dispatchEvent (new MessageEvent (
							this, MessageEvent.MessageType.Status,
							"Erzeugen sie erst die Teleport-Kante!"));
					return;
				}
			...
			EventServer.getInstance ().dispatchEvent (new MessageEvent (
					this, MessageEvent.MessageType.Error, ex.getLocalizedMessage ()));
		}
	}
}
