...
public class WindowsDirectoryChooserUI extends BasicFileChooserUI
    ...
    public void rescanCurrentDirectory(JFileChooser fc) {
        ...
        findFile(chooser.getSelectedFile() == null ? chooser.getCurrentDirectory() : chooser.getSelectedFile(),
                true, true);
    }
    ...
    private void updateView(JFileChooser chooser) {
        ...
        preferredSize = new Dimension(Math.max(preferredSize.width,
     ...
     * @param selectFile
     */
    private void findFile(File fileToLocate, boolean selectFile,
            boolean reload) {
    ...
    private class SelectionListener implements TreeSelectionListener {
        ...
        public void valueChanged(TreeSelectionEvent e) {
            getApproveSelectionAction().setEnabled(
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (!currentDirectory.canWrite()) {
                JOptionPane.showMessageDialog(
                        fc,
                        UIManager.getString("DirectoryChooser.cantCreateFolderHere"),
                        UIManager.getString("DirectoryChooser.cantCreateFolderHere.title"),
            ...
            String newFolderName = JOptionPane.showInputDialog(
                    fc,
                    UIManager.getString("DirectoryChooser.enterFolderName"),
                    newFolderText, JOptionPane.QUESTION_MESSAGE);
            if (newFolderName != null) {
                ...
                if (newFolder.mkdir()) {
                    ...
                    JOptionPane.showMessageDialog(
                            fc,
                            UIManager.getString("DirectoryChooser.createFolderFailed"),
                            UIManager.getString("DirectoryChooser.createFolderFailed.title"),
                            JOptionPane.ERROR_MESSAGE);
                }
            }
        }
    ...
    private class FileTreeNode extends LazyMutableTreeNode implements
            Comparable {
        ...
        public boolean canEnqueue() {
            return !isLoaded()
                    && !chooser.getFileSystemView().isFloppyDrive(
                    getFile())
                    && !chooser.getFileSystemView().isFileSystemRoot(
                    getFile());
        }
        ...
        private FileTreeNode[] getChildren() {
            File[] files = chooser.getFileSystemView().getFiles(
                    getFile(), chooser.isFileHidingEnabled());
    ...
    private static synchronized void addToQueue(FileTreeNode node,
            JTree tree) {
    ...
    private static final class Queue extends Thread {
        ...
        public void add(WindowsDirectoryChooserUI.FileTreeNode node,
                JTree tree) {
            if (!isAlive()) {
                throw new IllegalArgumentException(
                        "Queue is no longer alive");
            }
    ...
    private static final class QueueItem {
        ...
        public QueueItem(WindowsDirectoryChooserUI.FileTreeNode node,
                JTree tree) {
            this.node = node;
            this.tree = tree;
        }
