...
public class PreflowPushTest {
    ...
    @Test
    public void getValueOfMaximumFlow() {
    }

    @Test
    public void run() {
        Network network = new Network(6, 7);
        Node sink = network.getNode(0);
        Node source = network.getNode(1);
        Node a = network.getNode(2);
        Node b = network.getNode(3);
        Node c = network.getNode(4);
        Node d = network.getNode(5);
        Edge e = new Edge(0, source, a);
        Edge f = new Edge(1, a, b);
        Edge g = new Edge(2, a, c);
        Edge h = new Edge(3, b, d);
        Edge i = new Edge(4, c, d);
        Edge j = new Edge(5, d, sink);
        Edge k = new Edge(6, c, b);
        network.setEdge(e);
        network.setEdge(f);
        network.setEdge(g);
        network.setEdge(h);
        ...
        sinks.add(sink);
        //PreflowPush algo = new PreflowPush(network, capacities, sources, sinks);
        //algo.run();
